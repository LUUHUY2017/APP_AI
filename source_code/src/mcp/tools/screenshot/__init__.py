...
def take_screenshot(arguments = None):
    ...
    if display_id and isinstance(display_id, str):
        if display_id.lower() in ('main', '主屏', '主显示器', '笔记本', '内屏'):
            display_id = 'main'
        elif display_id.lower() in ('secondary', '副屏', '副显示器', '外接', '外屏', '第二屏'):
            ...
            try:
                ...
                logger.warning(f'''Invalid display parameter: {display_id}, using default''')
                display_id = None
                continue
