...
def _ensure_library_loaded():
    ...
    system = platform.system().lower()
    if system != 'darwin':
        raise RuntimeError(f'''WebRTC APM library is only supported on macOS, current platform: {system}. Windows and Linux should use system-level AEC instead.''')
...
class WebRTCAudioProcessing:
    ...
    def create_stream_config(self = None, sample_rate = None, num_channels = None):
        '''创建流配置。
    ...
    def destroy_stream_config(self = None, config_handle = None):
        '''销毁流配置。'''
        _lib.WebRTC_APM_DestroyStreamConfig(config_handle)

    
    def apply_config(self = None, config = None):
        '''将配置应用到音频处理模块。
    ...
    def process_reverse_stream(self, src = None, src_config = None, dest_config = None, dest = ('src', ctypes.Array, 'src_config', int, 'dest_config', int, 'dest', ctypes.Array, 'return', int)):
        '''处理反向流（渲染/播放音频）。
        ...
        Returns:
            状态码（0表示成功）
        '''
        return _lib.WebRTC_APM_ProcessReverseStream(self._handle, src, src_config, dest_config, dest)

    
    def process_stream(self, src = None, src_config = None, dest_config = None, dest = ('src', ctypes.Array, 'src_config', int, 'dest_config', int, 'dest', ctypes.Array, 'return', int)):
        '''处理采集流（麦克风音频）。
        ...
        Returns:
            状态码（0表示成功）
        '''
        return _lib.WebRTC_APM_ProcessStream(self._handle, src, src_config, dest_config, dest)

    
    def set_stream_delay_ms(self = None, delay_ms = None):
        '''设置流延迟（毫秒）。
...
def create_default_config():
    ...
    config.pre_amp.fixed_gain_factor = 1
    config.level_adjustment.enabled = False
    config.level_adjustment.pre_gain_factor = 1
    config.level_adjustment.post_gain_factor = 1
    ...
    config.gain_control1.analog_controller.predictor.clipping_threshold = -1
    config.gain_control1.analog_controller.predictor.crest_factor_margin = 3
    ...
    config.gain_control2.adaptive_controller.headroom_db = 5
    config.gain_control2.adaptive_controller.max_gain_db = 50
    config.gain_control2.adaptive_controller.initial_gain_db = 15
    config.gain_control2.adaptive_controller.max_gain_change_db_per_second = 6
    config.gain_control2.adaptive_controller.max_output_noise_level_dbfs = -50
    config.gain_control2.fixed_controller.gain_db = 0
    return config
